...
export default async function DashboardPage() {
  ...
  const uptimeDisplay =
    uptimePercent !== null ? `${uptimePercent.toFixed(1)}%` : null;
  ...
  const totalMap = Object.fromEntries(
    Array.isArray(uptimeCounts)
      ? uptimeCounts.map((r) => [r.monitorId, r._count.id])
      : [],
  );
  const successMap = Object.fromEntries(
    Array.isArray(uptimeSuccessCounts)
      ? uptimeSuccessCounts.map((r) => [r.monitorId, r._count.id])
      : [],
  );
  ...
  const STATUS_ORDER: Record<MonitorStatus, number> = {
    DOWN: 0,
    UNKNOWN: 1,
    PAUSED: 2,
    UP: 3,
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div
          className={cn(
            ...
            uptimeState === "great" &&
              "hover:shadow-[0_8px_30px_oklch(0.65_0.18_145_/_0.15)]",
            uptimeState === "poor" &&
            ...
            {prioritizedMonitors.map((monitor, idx) => (
              <Link
                ...
                className={cn(
                  ...
                  idx !== prioritizedMonitors.length - 1 &&
                  ...
                  <div className="relative h-1.5 flex-1 overflow-hidden rounded-full bg-muted">
                    <div
                      className={cn(
                        ...
                        monitor.uptime !== null && monitor.uptime > 80
                          ? "bg-success/70"
                          : "bg-destructive/60",
      ...
      {monitorCount === 0 && (
        <div className="flex flex-1 flex-col items-center justify-center rounded-xl border border-dashed border-border/60 py-16">
          ...
          <p className="mt-2 max-w-xs text-center text-sm text-muted-foreground">
            Monitors track the uptime and latency of your APIs. Add your first
            one to start seeing data here.
          </p>
          <Link
            href="/dashboard/monitors"
